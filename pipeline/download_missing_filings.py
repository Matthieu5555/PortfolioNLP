...
After running this, run:
    uv run python pipeline/fetch_filing_dates.py   # get actual SEC filing dates
    uv run python pipeline/embed_filings.py         # embed (auto-skips existing)

Usage:
    uv run python pipeline/download_missing_filings.py
    uv run python pipeline/download_missing_filings.py --dry-run  # just list tickers
"""
...
def main() -> None:
    ...
    if records:
        ...
        logger.info(
            "Next steps:\n"
            "  1. uv run python pipeline/fetch_filing_dates.py  (get SEC filing dates)\n"
            "  2. uv run python pipeline/embed_filings.py        (embed new tickers)"
        )
    else:
