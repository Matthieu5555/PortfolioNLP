...
Reads transcript text from data/tranpipeline/{ticker}/Q{q}_{year}.txt,
embeds each document (chunking + mean-pooling), and saves per-firm
embedding arrays to data/embeddings_tranpipeline/{ticker}.npz.
...
Usage:
    uv run python pipeline/embed_transcripts.py
    uv run python pipeline/embed_transcripts.py --force   # re-embed everything
"""
